...
if submit_chart:
    st.markdown(f"**{chart_type} Chart: Sales by Category**")
    agg_data = filtered.groupby("category", as_index=False)["sales"].sum()
    
    if chart_type == "Bar":
        fig = px.bar(agg_data, x="category", y="sales")
        st.plotly_chart(fig, use_container_width=True)
    elif chart_type == "Line":
        fig = px.line(agg_data, x="category", y="sales")
        st.plotly_chart(fig, use_container_width=True)
    elif chart_type == "Pie":
        fig = px.pie(agg_data, names="category", values="sales")
        st.plotly_chart(fig, use_container_width=True)
